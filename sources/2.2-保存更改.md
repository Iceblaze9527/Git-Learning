...
*.pdf diff=pdfconv
```
配置完成后，`git diff`将首先通过配置的转换器脚本运行二进制文件，并对转换器输出进行`diff`。 同样的方法可以从各种二进制文件中获取有用的差异，例如：`zips`，`jar`和其他存档：使用`unzip -l` （或类似）将显示已添加或删除的路：exiv2可用于显示元数据更改，例如图像尺寸文档：存在用于将.odf，.doc和其他文档格式转换为纯文本的转换工具。 在紧要关头，字符串通常适用于不存在正式转换器的二进制文件。
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTM4NDU1Njc4LC0zMjU1MzUzMTMsLTE5Mj
