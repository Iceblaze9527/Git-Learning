...
首先，需要指定一个`textconv`过滤器，描述如何将特定类型的二进制文件转换为文本。 我们使用一个名为`pdftohtml`的简单实用程序（可通过自制软件提供）将PDF转换为人类可读的HTML。 您可以通过编辑`.git/config`文件为单个存储库设置此选项，也可以通过编辑`~ /.gitconfig`全局`~ /.gitconfig`
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE3MjA1MDMyMTMsLTMyNTUzNTMxMywtMT
kyODg2Mzc1MCwtOTMwMTEzNjYsLTI4NDMzODI4NiwxOTMyOTI4
NjUyLC01MTYyNDU1MzBdfQ==
-->