...
```
此模式通过空格标记添加和删除的行，然后对其进行区分。<font color="red">输出仅显示已更改的颜色编码的单词。</font>
...
*.pdf diff=pdfconv
```
配置完成后，`git diff`将首先通过配置的转换器脚本运行二进制文件，并对转换器输出进行`diff`。 同样的方法可以从各种二进制文件中获取有用的差异，例如：`zips`，`jar`和其他存档;使用`unzip -l` （或类似）将显示在`commit`映像之间已添加或删除的路径；`exiv2`可用于显示元数据更改，例如图像尺寸文档；也存在用于将`.odf`，`.doc`和其他文档格式转换为纯文本的转换工具。 在紧要关头，字符串对于不存在正式转换器的二进制文件也to。
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjQ3NTU2MDA4LC0zMjU1MzUzMTMsLTE5Mj
g4NjM3NTAsLTkzMDExMzY2LC0yODQzMzgyODYsMTkzMjkyODY1
MiwtNTE2MjQ1NTMwXX0=
-->