...
```
Git确实有一个功能，允许您指定一个shell命令，在执行diff之前将二进制文件的内容转换为文本。 它确实需要一点设置。 首先，您需要指定一个textconv过滤器，描述如何将特定类型的二进制文件转换为文本。 我们使用一个名为pdftohtml的简单实用程序（可通过自制软件提供）将我的PDF转换为人类可读的HTML。 您可以通过编辑`.git/config`文件为单个存储库设置此选项，也可以通过编辑`~ /.gitconfig`全局`~ /.gitconfig`
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTM1MDYxNDMwLC0zMjU1MzUzMTMsLTE5Mj
