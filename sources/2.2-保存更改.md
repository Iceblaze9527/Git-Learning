...
>  
>  Modified by Leona Xiang on Aug 18, 2020
...
* `git add -p`: 开始交互式的缓存，你可以选择文件的一部分加入到下次提交缓存。它会向你展示一堆更改，等待你输入一个命令。`y` 将这块更改加入缓存，`n` 忽略这块更改，`s` 将它分割成更小的块，`e` 手动编辑这块更改，以及 `q` 退出。
...
### 2.2.3.1 用法
* `git commit`: 提交已经缓存的快照。它会运行文本编辑器，等待你输入提交信息。当你输入信息之后，保存文件，关闭编辑器，创建实际的提交。
* `git commit -m "<message>"`: 提交已经缓存的快照。但将 `<message>` 作为提交信息，而不是运行文本编辑器。
* `git commit -a`: 提交一份包含工作目录所有更改的快照。它只包含那些之前已经通过 `git add` 添加过的文件。
* `git commit --amend`: 此选项为commit命令添加了另一级功能。 传递该参数将修改上次提交而不是创建新的提交，更改将添加到以前的提交。 此命令将打开系统配置的文本编辑器，并提示更改先前指定的提交信息。
...
### 2.2.3.4 例子
下面这个例子假设你编辑了 `hello.py` 文件的一些内容，并且准备好将它提交到项目历史。首先，你需要用 `git add` 缓存文件，
...
在没有文件路径的情况下调用`git diff`将比较整个存储库中的更改。 默认情况下， `git diff`将显示自上次提交以来的任何未提交的更改。
...
### 2.2.4.3 比较二进制文件
除了我们到目前为止演示的文本文件实用程序之外，`git diff`可以在二进制文件上运行。 不幸的是，默认输出不是很有帮助。
```shell
git diff
Binary files a/script.pdf and b/script.pdf differ
```
`git`确实有一个功能，允许您指定一个`shell`命令，在执行`diff`之前将二进制文件的内容转换为文本， 但是需要一点设置。 

首先，需要指定一个`textconv`过滤器，描述如何将特定类型的二进制文件转换为文本。 我们使用一个名为`pdftohtml`的简单实用程序（可通过自制软件提供）将PDF转换为HTML。 可以通过编辑`.git/config`文件为单个存储库设置此选项，也可以通过编辑`~ /.gitconfig`文件为全局存储库设置此选项。
...
然后，你需要做的就是将一个或多个文件模式与的`pdfconv`过滤器相关联。 你可以通过在库的根目录中创建`.gitattributes`文件来完成此操作。

```shell
*.pdf diff=pdfconv
```
配置完成后，`git diff`将首先通过配置的转换器脚本运行二进制文件，并对转换器输出进行`diff`。 同样的方法可以从各种二进制文件中获取有用的差异，例如：`zips`，`jar`和其他存档;使用`unzip -l` （或类似）将显示在`commit`映像之间已添加或删除的路径；`exiv2`可用于显示元数据更改，例如图像尺寸文档；也存在用于将`.odf`，`.doc`和其他文档格式转换为纯文本的转换工具。 在紧要关头，字符串对于不存在正式转换器的二进制文件也通常是有效的。
...
## 2.2.6 .gitignore
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIxMTQxMDg1ODksMzM3MTk2MTI3LC05MD
Y1MjA0MDMsLTMyNTUzNTMxMywtMTkyODg2Mzc1MCwtOTMwMTEz
NjYsLTI4NDMzODI4NiwxOTMyOTI4NjUyLC01MTYyNDU1MzBdfQ
==
-->