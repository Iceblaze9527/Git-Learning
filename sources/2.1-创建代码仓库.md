...
git config --global alias.co checkout
```
创建别名不会修改源命令。 因此即使我们现在有`git co`别名， `git checkout`仍然可用。 这些别名是使用`--global`标志创建的，这意味着它们将存储在Git的全局操作系统级配置文件中。 在linux系统上，全局配置文件位于`/.gitconfig`的用户主目录中。
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTgyODMzNjUxMiwtMTgxMzIyOTk3NywxMj
Y2ODMwMzQ2XX0=
-->